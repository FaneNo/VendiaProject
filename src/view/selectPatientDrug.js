...
function EnhancedTableToolbar(props) {
  const { numSelected, handleSelectButton } = props;
...
export default function EnhancedTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      const drugData = await entities.drug.list();
      setDrug(drugData.items);
      setPatient(patientData);
      console.log(patientData, drugData.items);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
